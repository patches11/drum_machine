...
int currentBPM = 120;
int currentVolume = 6;  // 1-10; headphone: n*0.1, line out: 31-(n-1)*2 (13=loud, 31=quiet)
...
void drawIconVol(int ox, int oy, uint16_t c) {
  uint16_t ci = c ? SSD1306_BLACK : SSD1306_WHITE;
  display.fillRect     (ox+1,  oy+7,  4, 7, c);           // speaker box
  display.fillTriangle (ox+5,  oy+7,  ox+5, oy+13, ox+11, oy+3,  c);  // upper cone
  display.fillTriangle (ox+5,  oy+7,  ox+5, oy+13, ox+11, oy+17, c);  // lower cone
  display.drawLine     (ox+13, oy+5,  ox+17, oy+2,  c);   // upper wave
  display.drawLine     (ox+13, oy+10, ox+18, oy+10, c);   // mid wave
  display.drawLine     (ox+13, oy+15, ox+17, oy+18, c);   // lower wave
}
...
void drawDisplay() {
  ...
  switch (activeMode) {
    case MODE_KICK:  drawIconKick (0, 0, SSD1306_WHITE); break;
    case MODE_HIHAT: drawIconHihat(0, 0, SSD1306_WHITE); break;
    case MODE_RESET: drawIconReset(0, 0, SSD1306_WHITE); break;
    case MODE_CLICK: drawIconClick(0, 0, SSD1306_WHITE); break;
    case MODE_BPM:   drawIconBPM  (0, 0, SSD1306_WHITE); break;
    case MODE_VOL:   drawIconVol  (0, 0, SSD1306_WHITE); break;
  }

  const char* modeNames[] = { "KICK", "HIHAT", "RESET", "CLICK", "BPM", "VOL" };
...
void setup() {
  ...
  sgtl5000.volume(currentVolume * 0.1f);
  sgtl5000.lineOutLevel(31 - (currentVolume - 1) * 2);
  delay(500);
      ...
      if (!longPressHandled) {
        switch (activeMode) {
          ...
          case MODE_HIHAT:
            ...
            if (pattern[activeMode][pressedStep]) trigger(activeMode);
            drawLEDs();
            break;
          case MODE_RESET:
            memset(pattern, 0, sizeof(pattern));
